...
from GaussianNoiseGenerator import add_noise
...
def plot_graph(data):
    ...
    ax.set(xlabel='Standard deviation', ylabel='Accuracy')
    plt.savefig(fname=f'{OUT_DIR}/plot.png')
    plt.tight_layout()
    plt.show()


def extract_data(predictions, stddev):
    result = []
    for prediction in predictions:
        _, score, _, _ = prediction
        result.append([stddev, score])
    return result
...
def run(step):
    ...
    for i in range(1, step + 1):
        ...
        stddev = 1000. / step * i
        print(f'Noise level: {stddev}')
        add_noise(target=images, mean=0.0, stddev=stddev)
        predictions = classify_images(images)
        # plot_images(images, predictions, stddev)
        data.append(extract_data(predictions, stddev))
